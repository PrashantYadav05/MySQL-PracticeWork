...
SELECT PEOPLE.id, PEOPLE.name, TEMP.address FROM PEOPLE LEFT JOIN ( SELECT id, MAX(address) as address  FROM ADDRESS GROUP BY id ) AS TEMP ON PEOPLE.id = TEMP.id; 
